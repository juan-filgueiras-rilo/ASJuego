defmodule Jugador do
  ...
  def constructor(nombre, nivel, clase)
      when is_integer(nivel) and
             is_binary(nombre) do
    {nombre, nivel, clase, Clase.getVidaMax(clase, nivel)}
  end

  def constructor(nombre, nivel, clase, vida) do
  ...
  def saveWithClassName({nombre, nivel, clase, vida}) do
    %{
      "nombre" => nombre,
      "nivel" => nivel,
  ...
  def save({nombre, nivel, clase, vida}) do
    %{
      "nombre" => nombre,
      "nivel" => nivel,
  ...
  def getHechizosDisponibles({_, nivel, clase, _}) do
    Clase.getHechizosDisponibles(clase, nivel)
  end

  def getVida({_, _, _, vida}) do
    vida
  end

  def getNombre({nombre, _, _, _}) do
    nombre
  end

  def getNivel({_, nivel, _, _}) do
    nivel
  end

  def getClase({_, _, clase, _}) do
    clase
  end

  def aplicarHechizo(
        {nombrePropio, nivelPropio, clasePropia, vidaPropia},
        hechizo,
        {nombreEnemigo, nivelEnemigo, claseEnemigo, vidaEnemigo}
      ) do
    ...
    case Hechizo.getTipo(hechizo) do
      "DMG" ->
        {{nombrePropio, nivelPropio, clasePropia, vidaPropia},
         {nombreEnemigo, nivelEnemigo, claseEnemigo, vidaEnemigo - fuerza}}

      "CUR" ->
        {{nombrePropio, nivelPropio, clasePropia, vidaPropia + fuerza},
         {nombreEnemigo, nivelEnemigo, claseEnemigo, vidaEnemigo}}
  ...
  def aplicarHechizos(jugador, [], enemigo) do
    {jugador, enemigo}
  end

  def aplicarHechizos(jugador, hechizos, enemigo)
      when is_list(hechizos) do

defmodule Clase do
  ...
  def constructor(nombre, constVid, linVid, cuadVid, hechizos)
      when is_binary(nombre) and
             is_integer(constVid) and
             is_integer(linVid) and
             is_integer(cuadVid) and
             is_list(hechizos) do
  ...
  def getHechizos({_, _, _, _, hechizos}) do
    hechizos
  end

  def getNombre({nombre, _, _, _, _}) do
    nombre
  end

  def getHechizosDisponibles({_, _, _, _, hechizos}, nivel) do
  ...
  def save({nombre, constVid, linVid, cuadVid, hechizos}) do
    %{
      "nombre" => nombre,
      "constVid" => constVid,
      "linVid" => linVid,
